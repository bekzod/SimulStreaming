...
class PaddedAlignAttWhisper:
    def __init__(self, cfg: AlignAttConfig) -> None:
        ...
        self.enforced_language = cfg.language if cfg.language != "auto" else None
        ...
        if cfg.decoder_type == "greedy":
            ...
            if cfg.kenlm_path and cfg.lm_weight and cfg.lm_weight > 0.0:
                try:
                    ...
                    logger.warning(
                        f"KenLM could not be initialized: {e}. Proceeding without LM."
                    )
    ...
    def infer(self, is_last=False):
        ...
        elif self.enforced_language is not None:
            # Ensure language is enforced even if auto-detection was attempted
            logger.info(f"Enforcing language: {self.enforced_language}")
            if self.tokenizer.language != self.enforced_language:
                self.create_tokenizer(self.enforced_language)
                self.detected_language = self.enforced_language
                self.init_tokens()
        ...
        while (
            ...
            # Enforce language token if configured
            if (
                new_segment
                and current_tokens.shape[1] == self.sot_index + 1
                and self.enforced_language is not None
            ):
                # We're at the position right after SOT where language token should be
                language_token_id = (
                    self.tokenizer.sot
                    + 1
                    + list(self.tokenizer.all_language_codes).index(
                        self.enforced_language
                    )
                )
                logits[:, :, :] = -np.inf
                logits[:, :, language_token_id] = 0
                logger.debug(
                    f"Enforcing language token for {self.enforced_language}: {language_token_id}"
                )
            ...
            generation_progress_loop.append(
                ("most_attended_frames", most_attended_frames.clone().tolist())
            )
